...
- **Phase 2A (implemented/current private release):** five fixed production `APP_USERS_JSON` accounts, signed identity sessions,
...
| `lib/companyAssessment.ts` | Per-company apply-channel/trust verdicts (ADR 0107): batched LLM assessment, shared cache, job stamping |
...
| `evals/company-cases/` | Labeled company+posting → expected apply-channel/trust cases (ADR 0107) |
