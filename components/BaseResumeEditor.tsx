...
import { Save, CheckCircle, Sparkles, Plus, Trash2, AlertCircle } from 'lucide-react';
import type { ResumeDoc, ResumeWork, ResumeEducation, ResumeSkill, ResumeProject } from '@/lib/types';

/**
 * Editable base résumé (ADR 0024, Phase 1). Structured JSON Resume editor — the
 * source of truth that per-job tailoring later reframes. "Rebuild from résumé
 * text" runs the one-time LLM parse of profile.resume_text; everything is then
 * hand-editable. Bullets/keywords use one-per-line / comma textareas for speed.
 */
...
export default function BaseResumeEditor() {
  ...
  function setBasics(key: string, value: string) {
    update({ basics: { ...d.basics, [key]: value } });
  }

  async function rebuild() {
    if (doc && !confirm('Rebuild the base résumé from your résumé text? This replaces the current structured résumé below (your résumé text under Profile is unchanged).')) return;
    setParsing(true);
    setError(null);
    try {
  ...
  return (
    <div className="space-y-5">
      ...
      <Section
        title="Skills"
        onAdd={() => update({ skills: [...d.skills, { name: '', keywords: [] }] })}
        ...
        {d.skills.map((s, i) => (
          <Row key={i} onRemove={() => update({ skills: d.skills.filter((_, j) => j !== i) })}>
            <Input
              label="Group"
              value={s.name}
              onChange={(v) => update({ skills: patch(d.skills, i, { name: v }) })}
      ...
      <Section
        title="Experience"
        onAdd={() => update({ work: [...d.work, { highlights: [] }] })}
        addLabel="Add role"
      >
      ...
      <Section
        title="Education"
        onAdd={() => update({ education: [...d.education, {}] })}
      ...
      <Section
        title="Projects"
        onAdd={() => update({ projects: [...d.projects, { highlights: [] }] })}
...
function patch<T extends Listable>(list: T[], i: number, p: Partial<T>): T[] {
