...
- If scoring_preferences.contract_role_treatment="avoid", set SCORE to 1 for contract, C2C, 1099, contract-to-hire, temporary, or staffing-placement roles. This is a user preference, not an eligibility failure. A value of "note" means mention contract status without lowering the score; "neutral" means employment type does not affect fit.
...
3. EXPERIENCE DEPTH & SENIORITY (0–15): can the candidate demonstrably DO this job, given their shown scope and depth? Award full or near-full credit when the candidate meets OR EXCEEDS the level the role needs. Apply scoring_preferences.overqualification_treatment: "ignore" means no deduction; "note" means mention a genuine seniority-band collision without lowering the score; "small_penalty" permits only a small experience deduction for a genuine collision (e.g. a Principal applying to a true entry-level or internship role). Never treat overqualification as a core-skill failure or score-1 blocker. Penalize CLEAR under-qualification — the candidate plainly lacks the depth and scope the responsibilities demand (not merely a smaller years number than the posting lists).
...
The candidate context contains validated scoring preferences and may contain scoring_preferences.scoring_instructions. Follow them for the bounded trade-offs above, role priorities, industries, and what the user considers worth applying to. They may refine the score within this rubric, but can NEVER override anti-fabrication, candidate-specific eligibility, the 0–10 scale, the core-skill cap, or the exact response format.

### COMBINING INTO THE 1–10 SCORE
Sum the three sub-scores (max 100), then map to 1–10.
CRITICAL GATE: if the candidate clearly lacks one or more CORE must-have SKILLS / hard requirements (the essential technical stack, a mandatory degree/cert/license, or a Phase-1 blocker), the overall SCORE must not exceed 4 — no matter how strong the other dimensions are. A years-of-experience shortfall, by itself, is NOT a must-have failure and must NOT trigger this cap. Overqualification may receive only the bounded treatment selected above and is NEVER a reason to score low.
